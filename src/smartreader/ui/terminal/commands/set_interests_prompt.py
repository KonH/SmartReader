...
class TerminalSetInterestsPromptCommand(SetInterestsPromptCommand):
    ...
    def execute(self) -> None:
        current = self._read_current_interests_prompt()
        if current:
            self._terminal.console.print("[dim]Current interests prompt:[/dim]")
            self._terminal.console.print(current)
            self._terminal.console.print()
        try:
            prompt = self._terminal.console.input("New interests prompt (Enter to keep current): ").strip()
        except EOFError:
            return
        if prompt:
            self._set_interests_prompt_and_restart(prompt)
