...
- **`TriggerParams`** still exists in `types/params.py` but is now internal to `TelegramUI.loop()` — the Coordinator no longer dispatches on mode. `TelegramSharedUIState` holds all Telegram-specific state (`trigger_queue`, `category_queue`, `add_step_queue`, `in_add_mode`, `in_skip_mode`, `in_set_cron_mode`, `waiting_for_category`, etc.).
...
- **Telegram multi-input collection**: when a command needs to collect an arbitrary number of messages (e.g. ban/skip word lists), use a `while True` loop on `add_step_queue.get()` with a "✅ Done" inline button that puts the sentinel `"__done__"` and a Cancel button that puts `None`; the `in_*_mode` flag stays `True` for the full duration of the loop. Always drain the queue (`while not q.empty(): q.get_nowait()`) before entering the loop — stale values from a previous interaction cause the flow to silently short-circuit.
...
- **Telegram stuck state**: if `async_send` fails before a value is placed on `add_step_queue`, `in_*_mode` stays `True` and the loop blocks on `add_step_queue.get()` indefinitely; only a process restart recovers.
...
- **`_KNOWN_COMMAND_TYPES` in `__main__.py`**: whitelist of abstract command base classes. New abstract commands (e.g. `BanWordCommand`) **must** be added here or the concrete subclass is silently filtered out and never instantiated.
