...
# [scoring.category_prompts]      # optional: EVAL prompt overrides keyed by category
# technology = "..."
# [scoring.channel_prompts]       # optional: EVAL prompt overrides keyed by source name
# tech_channel = "..."
# Resolution for openai_score EVAL: stage > channel > category > global
...
| `telegram_shown_content` | `{"items": list[dict]}` — serialized `Content` shown in Telegram (newest last, capped at 200); lets inline votes resolve after later shows and restarts |
...
- **`mode_state: UIMode`** (defined in `ui/telegram/state.py`): single `Literal["", "add", "edit", "remove", "skip", "ban", "prompt", "group", "sources", "cron", "config"]` field replaces individual `in_*_mode` booleans. Guards: `on_trigger` checks `if s.mode_state != "": return`; `on_add_message` checks `if s.mode_state == "": return`. Commands set `mode_state = "<mode>"` at entry and `mode_state = ""` on exit (preferably in `try/finally`).
- **Interactive Telegram flows**: multi-step conversations (add/edit/remove source, skip word) block inside the Telegram command `execute()` methods using `add_step_queue`; `mode_state` on `TelegramSharedUIState` prevents trigger commands from interrupting. Cancel buttons put `None` on the queue; text messages put the typed value.
- **Telegram sources menu**: the main action menu shows a single **SOURCES** button (`SourcesGroupCommand` / `TelegramSourcesGroupCommand`) that offers Add / Edit / Remove sub-commands. `/add` remains a shortcut to add a source directly.
...
- **Telegram inline votes must work on any earlier message**: `content_by_id` on `TelegramSharedUIState` is accumulated across shows (never replaced per show) and persisted via `ui/telegram/shown_content.py` (`remember_shown_content` / `load_shown_content`, restored at the start of `TelegramUI.loop`). The vote handler edits the clicked message via `event.edit(...)` — do not track message ids per show. Live feedback goes to `app_state.pipeline` (the Coordinator's own pipeline ref goes stale after `rebuild_pipeline()`).
